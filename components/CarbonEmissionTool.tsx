...
import { TextInput, SelectInput, SearchableSelect, FieldLabel } from './FormControls';
...
// EU default values for Taiwan (IR 2025/2621 Annex I as corrected by IR 2026/1740), generated by
// tools/gen_default_values.py. Totals are the regulation's raw values; the mark-up is applied below.
const DEFAULT_GOODS = new Map(DEFAULTS.goods.map(g => [g.cn, g]));
const DEFAULT_OPTIONS = DEFAULTS.goods.map(g => ({
    value: g.cn,
    label: `${g.cn} – ${g.description.length > 70 ? g.description.slice(0, 68) + '…' : g.description}`,
}));
...
const CarbonEmissionTool: React.FC<{ e62Rows?: KeyValue[] }> = ({ e62Rows = [] }) => {
    ...
    const [cmp, setCmp] = useState({ tonnes: '', cn: '', defaultSee: '', actualSee: '', year: '2026' });
    ...
    // A picked good decides the sector; a hand-typed default falls back to the installation's goods.
    const picked = DEFAULT_GOODS.get(cmp.cn);
    const fertiliser = picked ? picked.sector === 'Fertilisers' : allFertiliser;
    const markup = fertiliser ? FERTILISER_MARKUP : (MARKUP[cmp.year] ?? 0.30);
    // Annex I lists the marked-up defaults to 3 decimals, so round the same way.
    const markedUpDefault = Math.round(num(cmp.defaultSee) * (1 + markup) * 1000) / 1000;
    const extraPerTonne = Math.max(0, markedUpDefault - num(cmp.actualSee));
    ...
    const handleExport = async () => {
        ...
        try {
            ...
            if (picked) ws.addRow(['', 'CN 碼', `${picked.cn} ${picked.description}`]);
            const c1 = ws.addRow(['', '出口量 (t)', num(cmp.tonnes)]); input(c1.getCell(3));
            const c2 = ws.addRow(['', picked ? `歐盟台灣預設值（總排放，tCO2e/t，${DEFAULTS.date} 版）` : '歐盟預設值（總排放，tCO2e/t）', num(cmp.defaultSee)]); input(c2.getCell(3));
            ...
            c5.getCell(3).value = { formula: `C${c1.number}*MAX(0,ROUND(C${c2.number}*(1+C${c3.number}),3)-C${c4.number})*${gap}` };
            result(c5.getCell(3));
            ws.addRow([]);
            ...
            [
                ...
                ['電網排放係數', `本表使用 ${num(gridFactor)} kgCO2e/kWh（預設為能源署公告值）。歐盟公告的台灣預設值為 0.561（IR 2025/2621 附件 II），若進口商採用請改為該值。`],
                ...
                ['預設值加成', 'IR 2026/1740（更正 IR 2025/2621）：水泥、鋼鐵、鋁、氫 2026 +10%、2027 +20%、2028 起 +30%；肥料一律 +1%。加成後四捨五入到小數第 3 位，與附件 I 相同；附件 I 以未四捨五入的總排放計算，可能相差 ±0.001。'],
                ['預設值數據', `選擇 CN 碼時，台灣預設值取自歐盟官方 Excel 第 ${DEFAULTS.version} 版（${DEFAULTS.date}，IR 2025/2621 附件 I，經 IR 2026/1740 更正）。台灣欄位為「–」的產品，依附件 I 改用「其他國家和地區」的值。Excel 僅供參考，法律效力以法規本文為準。`],
    ...
    return (
        <div className="space-y-6">
            ...
            <Group>
                ...
                <p className="mt-3 text-xs text-slate-500">{t('電網係數預設為能源署公告的 0.474 kg/度。歐盟公告的台灣預設值是 0.561（IR 2025/2621 附件 II），進口商若採用它，請改成 0.561。', 'The grid factor defaults to 0.474 kg/kWh as published by Taiwan’s Energy Administration. The EU default for Taiwan is 0.561 (IR 2025/2621 Annex II); use it if your importer does.')}</p>
            </Group>
            ...
            <Group>
                ...
                <p className="mb-4 text-xs text-slate-500">{t('沒有經查證的實際數據時，進口商只能用歐盟預設值，而且要再加成（2026 年 10%、2027 年 20%、2028 年起 30%；肥料 1%，依 IR 2026/1740）。選擇產品 CN 碼，會自動帶入歐盟公告的台灣預設值（' + DEFAULTS.date + ' 更正版）。',
                    'Without verified actual data the importer must use the EU default value plus a mark-up (10% in 2026, 20% in 2027, 30% from 2028; fertilisers 1%, per IR 2026/1740). Pick the product’s CN code to fill in the EU default for Taiwan (corrected version of ' + DEFAULTS.date + ').')}</p>
                <div className="mb-4">
                    <SearchableSelect label={t('產品 CN 碼（自動帶入預設值）', 'Product CN code (fills in the default)')} id="sc-cn" options={DEFAULT_OPTIONS} value={cmp.cn}
                        onChange={v => setCmp(c => ({ ...c, cn: v, defaultSee: DEFAULT_GOODS.has(v) ? String(DEFAULT_GOODS.get(v)!.total) : c.defaultSee }))} />
                    {picked?.fromOther && <p className="mt-1 text-xs text-slate-500">{t('歐盟沒有訂台灣這項產品的數值，依法規改用「其他國家和地區」的預設值。', 'The EU sets no Taiwan value for this good, so the “Other countries and territories” default applies.')}</p>}
                </div>
                <div className="grid grid-cols-1 gap-4 md:grid-cols-4">
                    ...
                    <TextInput label={t('歐盟預設值（總排放）', 'EU default (total)')} id="sc-d" type="number" unit="tCO₂e/t" value={cmp.defaultSee} onChange={e => setCmp(c => ({ ...c, defaultSee: e.target.value, cn: '' }))} />
